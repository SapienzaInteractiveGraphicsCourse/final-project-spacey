...
let bonesOffset = [];
let actualBones = [];
let startingPosition = new BABYLON.Vector3(0, 100, 0);
let targetPosition = new BABYLON.Vector3(20, 20, 150);
let farCameraPosition = new BABYLON.Vector3(0, 100, 0);
let farCamera;
let nearCamera;
let ground;
let boy;
...
let txt = "MISSION 2: The rover fell down the Mars Valles, find it and fix it.";
...
let createScene = function () {
    ...
    farCamera = new BABYLON.FreeCamera("farCamera", farCameraPosition, scene);
    ...
    nearCamera = new BABYLON.ArcRotateCamera("nearCamera", 0, 0, 0, farCamera.position, scene);
    ...
    let hemiLight = new BABYLON.HemisphericLight("hemi", new BABYLON.Vector3(0, 100, -200), scene);
    hemiLight.intensity = 0.2;
    hemiLight.diffuse = new BABYLON.Color3(1, 1, 1);
    hemiLight.specular = new BABYLON.Color3(0.6, 0.6, 0.5);
    hemiLight.groundColor = new BABYLON.Color3(0, 0, 0);

    dirLight = new BABYLON.DirectionalLight("sun", new BABYLON.Vector3(0, -1, 2), scene);
    dirLight.intensity = 0.8;
    dirLight.diffuse = new BABYLON.Color3(1, 0.57, 0);
    dirLight.specular = new BABYLON.Color3(1, 0.9, 0);
    dirLight.ambient = new BABYLON.Color3(1, 0.73, 0);
    ...
    godrays.mesh.position = new BABYLON.Vector3(150, 100, -150);
    ...
    let skybox = BABYLON.Mesh.CreateSphere("galaxy", 20.0, 500.0, scene);
...
function slowZoom() {
    ...
    movein_keys.push({
        frame: 0,
        value: farCameraPosition.clone().add(new BABYLON.Vector3(-100, -25, 200))
    });
    movein_keys.push({
        frame: 400,
        value: farCameraPosition.clone().add(new BABYLON.Vector3(-100, -25, 160))
    });

    movein.setKeys(movein_keys);

    farCamera.animations = [];
    farCamera.animations.push(movein);

    scene.beginAnimation(farCamera, 0, 400, false, 1);
}
function fastZoom() {
    ...
    movein_keys.push({
        frame: 0,
        value: farCamera.position.clone()
    });
    movein_keys.push({
        frame: 180,
        value: startingPosition.clone().add(new BABYLON.Vector3(0, 2, 6))
    });
    movein_keys.push({
        frame: 200,
        value: startingPosition.clone().add(new BABYLON.Vector3(0, 2, 4))
    });
    ...
    scene.beginAnimation(farCamera, 0, 200, false, 1, function () {
        fading(rect2, 30, 0, 1);
        nearCamera.position = farCamera.position;
...
function showGUI() {
    ...
    mapImage = new BABYLON.GUI.Image("", "../images/heightmars.png");
    mapImage.width = 1.1;
    mapImage.height = 1.1;
    mapImage.alpha = 0.5;
    ...
    mapImage.onImageLoadedObservable.addOnce(function () {
        ...
        pos2.left = targetPosition.x / widthGround * mapImage.widthInPixels / 4
        pos2.top = targetPosition.z / heightGround * mapImage.heightInPixels / 4
    })
